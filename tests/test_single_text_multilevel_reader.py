...
def test_single_text_multilevel_reader_uses_text_field_and_manifest(tmp_path):
    ...
    (tmp_path / "train.json").write_text(json.dumps([{
        "id": "sample-1", "text": "One input only.",
...
def test_single_text_multilevel_reader_requires_text(tmp_path):
    ...
    (tmp_path / "train.json").write_text(json.dumps([{
        "id": "sample-1", "annotation_labels": {"level1": [0, 0], "level2": [0, 0], "level3": [0, 0]},
    }]), encoding="utf-8")

    try:
        SingleTextMultilevelJSONReader(data_path=str(tmp_path)).load_train()
