...
from nli_toolkits.data.readers.base import NLIDistributionReader
...
class ChaosNLIReader(NLIDistributionReader):
    ...
    def _infer_default_data_path(self) -> Tuple[Path, str]:
        source = (self.source or "").lower()
        if "alpha" in source:
            return Path("chaosNLI_v1.0/chaosNLI_alphanli.jsonl"), "chaosNLI-alphaNLI"
        if "mnli" in source:
            return Path("chaosNLI_v1.0/chaosNLI_mnli_m.jsonl"), "chaosNLI-MNLI"
        return Path("chaosNLI_v1.0/chaosNLI_snli.jsonl"), "chaosNLI-SNLI"
    ...
    def load_split(self, split: Split) -> List[NLIDistributionSample]:
        ...
        if data_path is None:
            default_path, pretty_name = self._infer_default_data_path()
            if default_path.exists():
                data_path = default_path
            else:
                raise ValueError(
                    "data_path must be provided (or place the official ChaosNLI v1.0 files under "
                    f"`{default_path.parent}` so a default `{pretty_name}` path can be inferred). "
