#!/usr/bin/env bash
# GPU-aware cross-dataset queue with durable logs and Ctrl-C cleanup.
set -euo pipefail
...
if ! flock -n 9; then
  echo "This eager part is already running; refusing a duplicate launch." >&2
  exit 1
fi
...
next=0; failed=0; peak_job_mb=0; last_launch=0
...
can_launch() {
  ...
  now=$(date +%s); (( now - last_launch >= warmup_seconds )) || return 1
  ...
  required=$((min_free_mb + peak_job_mb * safety_pct / 100)); (( free >= required ))
}
